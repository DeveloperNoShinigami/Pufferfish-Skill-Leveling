...
|-------|------|----------|---------|-------------|
| `targets` | string[] | **Yes** | — | Loot table IDs to inject into. |
| `chance` | float | No | `1.0` | Probability (0.0–1.0) that this group triggers at all. |
| `rolls` | object | No | `{ "min": 1, "max": 1 }` | How many items to generate per trigger. |
| `rolls.min` | integer | No | `1` | Minimum rolls. |
| `rolls.max` | integer | No | `1` | Maximum rolls (inclusive). |
...
| Field | Type | Required | Default | Description |
|-------|------|----------|---------|-------------|
| `type` | string | No | `"item"` | Entry type — see [Entry Types](#-entry-types). |
| `name` | string | **Yes** | — | Item registry ID. |
| `weight` | integer | No | `1` | Relative probability within the group (higher = more likely). |
| `chance` | float | No | `1.0` | Per-entry drop chance (rolled after the entry is selected). |
| `min_level` | integer | No | `1` | Minimum level for `skill_tome` entries. |
| `max_level` | integer | No | `1` | Maximum level for `skill_tome` entries. |
| `nbt` | string | No | — | Optional NBT data string. |
| `skill` | string | No | — | Specific skill ID for `skill_tome` entries. |
| `skills` | array | No | `[]` | Weighted skill pool for `skill_tome` entries. |
