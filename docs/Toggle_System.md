...
| `duration` | integer | — | Duration in ticks. Use `-1` for infinite. |
| `amplifier` | integer | `0` | Effect level (0 = Level I). |
| `persistent` | boolean | `false` | Survives death when `true`. |
| `is_protected` | boolean | `false` | Re-applies automatically if cleared by milk/death. |
