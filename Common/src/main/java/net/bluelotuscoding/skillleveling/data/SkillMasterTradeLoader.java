...
public class SkillMasterTradeLoader extends JsonDataLoader {
        ...
        prepared.forEach((id, element) -> {
            try {
                if (element.isJsonObject()) {
                    parseAndAddTemplate(element.getAsJsonObject());
                } else if (element.isJsonArray()) {
                    element.getAsJsonArray().forEach(item -> {
                        if (item.isJsonObject()) {
                            parseAndAddTemplate(item.getAsJsonObject());
                        }
                    });
                }
            } catch (Exception e) {
    ...
    private void parseAndAddTemplate(JsonObject json) {
        if (json.has("tier")) {
    ...
    private TradeTemplate parseTemplate(JsonObject json) {
        ...
        return new TradeTemplate(input1, input2, output, maxUses, experience, multiplier, chance);
    }
