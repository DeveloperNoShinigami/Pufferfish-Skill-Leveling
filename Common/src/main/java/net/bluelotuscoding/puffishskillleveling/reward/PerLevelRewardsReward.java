...
import net.bluelotuscoding.puffishskillleveling.PuffishSkillLeveling;
...
/**
 * Reward that grants nested rewards based on the player's current skill level.
 *
 * <p>The configuration maps level numbers to lists of rewards that become active
 * once the skill reaches that level. All active rewards stack.</p>
 */
public class PerLevelRewardsReward implements Reward {
    public static final ResourceLocation ID = SkillsMod.createIdentifier("per_level_rewards");
    ...
    public void update(RewardUpdateContext context) {
        ...
        // Grant configured points for each newly reached level.
        int clampedNew = Math.min(newCount, maxLevel);
        int clampedOld = Math.min(oldCount, maxLevel);
        int gainedLevels = clampedNew - clampedOld;
        if (gainedLevels > 0 && pointsPerLevel > 0 && skillId != null) {
            int points = gainedLevels * pointsPerLevel;
            var sourceId = new ResourceLocation(PuffishSkillLeveling.MOD_ID, "per_level_rewards");
            SkillsAPI.streamCategories()
                    .filter(cat -> cat.getSkill(skillId).isPresent())
                    .findFirst()
                    .ifPresent(cat -> cat.addPoints(player, sourceId, points));
        }
        ...
        for (var entry : levelRewards.entrySet()) {
            ...
            for (var reward : entry.getValue()) {
                reward.getInstance().update(new RewardUpdateContextImpl(player, count, action));
            }
        }
    }
