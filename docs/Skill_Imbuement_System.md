...
|-------|------|-------------|
| `imbue_chance` | float | Probability (0.0–1.0) that an eligible item gets imbued. |
| `max_skills` | integer | Maximum number of skills per item. |
| `min_level` | integer | Minimum skill level to roll. |
| `max_level` | integer | Maximum skill level to roll. Clamped to skill's `max_skill_level`. |
...
| Field | Type | Description |
|-------|------|-------------|
| `enabled` | boolean | Enables/disables distance scaling. |
| `origin` | [x, z] | Center point for distance calculation. |
| `brackets` | array | Sorted by distance. First matching bracket sets the cap. |
| `brackets[].distance` | integer | Distance threshold in blocks. |
| `brackets[].max_level` | integer | Maximum skill level within this radius. |
| `brackets[].chance_mult` | float | Multiplier on imbue chance at this distance (reserved for future use). |
...
| `weight` | integer | Relative probability. Higher = more likely. |
...
|-------|------|---------|-------------|
| `imbue_chance` | float | -1 (use dimension) | Override imbue probability. |
| `min_level` | integer | -1 (use dimension) | Override minimum level. |
| `max_level` | integer | -1 (use dimension) | Override maximum level. |
| `max_skills` | integer | -1 (use dimension) | Override max skills per item. |
